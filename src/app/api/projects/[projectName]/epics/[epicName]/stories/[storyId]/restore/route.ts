...
/**
 * POST /api/projects/[projectName]/epics/[epicName]/stories/[storyId]/restore
  ...
  try {
    ...
    if (!story.archived) {
      return NextResponse.json(
        {
          success: false,
          error: `Story "${storyId}" is not archived`,
        },
        { status: 400 }
      )
    }

    // Update story to remove archived flag
    const updatedStory = {
      ...story,
      archived: false,
    ...
    return NextResponse.json({
      success: true,
      data: updatedStory,
      message: `Story "${storyId}" restored successfully`,
    })
  } catch (error) {
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to restore story',
      },
      { status: 500 }
    )
  }
